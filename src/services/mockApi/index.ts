...
import * as newsHandlers from './news.mock';
...
export const mockApi = {
  async get<T = any>(url: string, config?: any): Promise<T> {
    ...
    if (url.startsWith('/news')) {
      return newsHandlers.get(url, config);
    }
  ...
  async post<T = any>(url: string, body?: any): Promise<T> {
    ...
    if (url.startsWith('/news')) {
      return newsHandlers.post(url, body);
    }
  ...
  async put<T = any>(url: string, body?: any): Promise<T> {
    ...
    if (url.startsWith('/news/')) {
      return newsHandlers.put(url, body);
    }
